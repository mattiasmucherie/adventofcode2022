...
const LOWEST_POINT = Array.from(CAVE_STONES)
  .map((el) => el.split(",").map((el) => parseInt(el)) as Coord)
...
export const partOne = () => {
  ...
  while (currentSandUnitPosition[1] < LOWEST_POINT) {
    const possibleMoves = getPossibleMoves(currentSandUnitPosition).filter(
      (el) => canMove(el, new Set([...CAVE_STONES, ...sandRestingUnits]))
    );
...
export const partTwo = () => {
  ...
  while (!occupied.has(SAND_START.join(","))) {
    const possibleMoves = getPossibleMoves(currentSandUnitPosition).filter(
      (el) => canMove(el, occupied, LOWEST_POINT + 2)
    );

    // Sand has no way to move any more and need to rest
    if (possibleMoves.length === 0) {
      occupied.add(currentSandUnitPosition.join(","));
      currentSandUnitPosition =
        path.length > 0 ? (path.pop() as Coord) : [...SAND_START];
      spawnedSands++;
      continue;
    }
...
export function getCave() {
  ...
  const lineCords = file
    ...
    .map((line) =>
      line
        .split(" -> ")
        .map((cord) => cord.split(",").map((el) => parseInt(el)) as Coord)
    );
